...
| **gitops** | _(新增)_ | delivery | commit + push | 不修改代码 |
...
Router: 分析 → Feature 任务 → 自动依次调用:
        Architect → Planner → Builder → Code Reviewer → Quality Auditor → Gitops
```
...
|-----------|-----------|---------|
| **Feature** | Architect → Planner → Builder → Code Reviewer → Quality Auditor → Gitops | 用户描述新功能 |
| **Feature + UI** | Architect → UI Designer → Planner → Builder → Code Reviewer → Quality Auditor → Gitops | 涉及 UI 变更 |
| **Bug** | Builder → Code Reviewer → Gitops | 跳过 Architect |
| **Incoming** | Router triage → Builder → Code Reviewer → Gitops | 先分类再处理 |
| **Prototype** | Prototyper | 独立使用，不进 delivery 链 |
| **Architecture health** | Architect → Builder → Code Reviewer → Gitops | Architect 做 grill + 设计 |
...
| **Gitops** | commit + push | All tasks requiring commit |
...
|-------|--------|------------|------|
| **Phase 1** | architect, planner, builder, code-reviewer, gitops | gitops skill | 覆盖主 delivery 链路 |
...
2. **Gitops 是否需要支持 squash merge、rebase 等高级操作？**
